...
public class Selenium {
     ...
     */
    public void createPlaceDetail(Station station, String category) {
        String keyword = null;
        switch (category) {
            case "restaurant":
                keyword = "식당";
    ...
    public Object[] placeTimeAndDetail(WebDriver driver) throws InterruptedException {
        ...
        // 현재 정보 요소
        for (WebElement infoElement : infos) {
                    ...
                    if (infoClassAttribute.contains("J1zN9")) { // 영업 시간이 등록되지 않은 경우 - 월 ~ 일 모두 동일한 시간 임의로 부여
                        ...
                        if (timeElement.get(1).findElement(By.cssSelector("div.y6tNq > span.A_cdD > span.i8cJw")).getText().contains("매일")) { // 운영시간이 "매일"로 등록된 경우
                            for (WebElement time : timeElement) {
                                if (time.getAttribute("class").contains("vI8SM") || time.getAttribute("class").contains("DzD3b")) {
                                } // 오늘 운영시간 정보인 경우
                                else if (time.getAttribute("class").contains("yN6TD")) {
                                } // 매장 휴무 정보인 경우
                                else {
                                    for (int day = 1; day <= 7; day++) {
     ...
     */
    public List<String> getPlaceDetailByCrawling(String station, String keyword) {
